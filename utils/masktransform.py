...
class WDTransformer(torch.nn.Module):
    def __init__(self, shape=(436, 1024), channel_in=3, use_l2=False, **unfold_args):
        """
    ...
    def forward(self, x, y):
        ...
        x = x - torch.tensor(list(range(self.kernel_size)) * self.kernel_size)
        y = y - torch.tensor([*chain.from_iterable([[i] * self.kernel_size for i in range(self.kernel_size)])])

        x = x.view(batch_size, -1, self.kernel_size ** 2).transpose(2, 1)
        y = y.view(batch_size, -1, self.kernel_size ** 2).transpose(2, 1)

        xy = torch.cat((x, y), 1)
        xy = self.fold(xy.float())  # this is an approx flow per kernel
        # xy = xy  # standardize by max value
        h, w = self.shape_in
        grid = torch.stack(torch.meshgrid([torch.linspace(-1, 1, w), torch.linspace(-1, 1, h)])).permute(2, 1,
                                                                                                         0).unsqueeze(0)
        xy = xy.permute(0, 2, 3, 1) / torch.tensor([436., 1024.]) + grid.float()
        return xy
