...
class SintelDataset(Dataset):
    ...
    def __init__(self, root_dir, shape=(256, 256), use_l2=True, channel_in=3, stride=1, kernel_size=2, transform=None, usecst = True):
        """
        Args:
        ...
        self.cst = censustransform() if usecst else lambda x: x
    ...
    def __getitem__(self, idx):
        ...
        sample = {'frame1': frame1,
                  'frame2': frame2, }
        if self.transform:
            sample['frame1'] = self.transform(self.cst(sample['frame1']))
            sample['frame2'] = self.transform(self.cst(sample['frame2']))
